...
function openCollection(id){
 ...
 $("#collectionContent").innerHTML=`<div class="page-heading"><p class="eyebrow">COLLECTION</p><h1>${esc(s.name)}</h1><p>${esc(s.description)}</p></div>
...
$("#productForm").onsubmit=async ev=>{ev.preventDefault();try{message("Uploading product photos…");const urls=[];for(const file of [...$("#productImages").files])urls.push(await uploadFile(file,"products"));const{data:p,error}=await sb.from("products").insert({section_id:+$("#productCollection").value,name:$("#productName").value,description:$("#productDescription").value||null,product_link:$("#productLink").value,image_url:urls[0],is_pinned:$("#productPinned").checked,display_order:+$("#productOrder").value||0}).select().single();if(error)throw error;const{error:imageError}=await sb.from("product_images").insert(urls.map((u,n)=>({product_id:p.id,image_url:u,display_order:n})));if(imageError)throw imageError;ev.target.reset();message("Product added with all photos.");renderAdmin()}catch(err){message(err.message)}};
